...
* **Dynamic Content Filtering:** Multi-parameter filtration supporting partial text title matches, specific category lookups, and flexible date-range scopes.
* **Live Financial Analytics Dashboard:** Real-time summary computing the current month's total spending and breaking it down dynamically by categories using database aggregation (`Sum`).

### What's Skipped (And Why)
* **Authentication & Multi-Tenancy:** Intentionally bypassed because the project brief explicitly specified *"Authentication or multi-user support is not cared about."* Omitting this allowed full focus on a bulletproof single-user application core.
* **Automated Test Suite:** Omitted due to the strict 2-hour implementation constraint, prioritizing manual validation of core end-to-end user workflows.
